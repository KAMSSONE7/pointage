...
def page_profil(page: Page, name=None):
    ...
    navigation_bar = CupertinoNavigationBar(
        ...
        inactive_color=other_color2,
        active_color=primary_color,
        on_change=navigation_changed,
        border=Border(top=BorderSide(1, shadow_color)),
        destinations=[
            NavigationBarDestination(
                icon=Icon(Icons.HOME_ROUNDED, color=text_secondary),
                selected_icon=Icon(Icons.HOME_ROUNDED, color=primary_color),
                label="Accueil"
            ),
            NavigationBarDestination(
                icon=Icon(Icons.CALENDAR_TODAY, color=other_color2),
                selected_icon=Icon(Icons.CALENDAR_TODAY, color=other_color2),
                label="Emploi du temps"
            ),
            NavigationBarDestination(
                icon=Icon(Icons.SHOW_CHART, color=other_color2),
                selected_icon=Icon(Icons.SHOW_CHART, color=other_color2),
                label="Statistiques"
            ),
            NavigationBarDestination(
                icon=Icon(Icons.PERSON_2, color=other_color2),
                selected_icon=Icon(Icons.PERSON_2, color=other_color2),
                label="Profil"
            ),
        ],
    )
    ...
    header = create_gradient_container(
        Column(
            controls=[
                ...
                Text(
                    user.get("profession", "Enseignant"),
                    size=16,
                    color=text_light,
                    text_align=TextAlign.CENTER,
                    opacity=0.9
    ...
    def deconnecter(e):
        ...
        dlg_modal = AlertDialog(
            modal=True,
            title=Text("Confirmation", color=text_primary),
            content=Text("Êtes-vous sûr de vouloir vous déconnecter ?", color=text_secondary),
            actions=[
                TextButton(
                    "Oui", 
                    on_click=confirmer_deconnexion,
                    style=ButtonStyle(color=other_color)
                ),
                TextButton(
                    "Non", 
                    on_click=annuler_deconnexion,
